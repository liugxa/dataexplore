...
define([
	...
	"dojo/i18n!./nls/FlipCard"
		...
		FlipCardStoreModel, FlipCardNavModule, FlipCardNavTreeModule, _FlipCardNavBase
	){	
	// module:
	...
	*/ 
	...
	return declare("idx/layout/FlipCardNavDynamic", [_FlipCardNavBase], {
         ...
         */
		genGridxNavigator: function(  cItems, itemLabel, itemNode  ){
            ...
            gridxNav.connect(gridxNav.body, 'onBuildCustomRow', lang.hitch(this, function(row, output){
                var rowItem = row.item();
                output[row[this.idProperty]] = "<div class='gridxCell' colid='" + this.labelAttr + "'>" + rowItem[this.labelAttr] + "</div>";
            }));
		...
		createSecondaryNavItems: function( cItems, itemId, itemLabel, itemNode ){
			var gridxNav = this.genGridxNavigator( cItems, itemLabel, itemNode );
		 ...
		 * 
		 */
		createRootNavItem: function( item ){
			//
			...
			var navListModel = this.navListModel,
				...
				itemNode = this.createMenuItemInNavigationBar(item);
