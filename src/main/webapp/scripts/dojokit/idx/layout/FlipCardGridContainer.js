...
define([
	...
	var FlipCardGridContainer = declare("idx/layout/FlipCardGridContainer", [_GridContainerLite], {
        ...
        postCreate: function(){
            ...
            //dnd drag and drop
            this.subscribe("/dojox/mdnd/drag/start", "gridContainerDndStart");
            this.subscribe("/dojox/mdnd/drop", "gridContainerDndEnd");
        },
        ...
        getItemPosIndex: function(item){
            var node = item.domNode, position = 0, maxRow = node.parentNode.children.length;
            
            while(node.previousSibling){
                position++;
                node = node.previousSibling;
            }
            position = maxRow - 1 - position;
            item.set("p", position);
            return position;
        }
